 ...
 * result = zlib's data_type (1 byte: 0 binary, 1 text, 2 unknown) || the raw stream; op 'i': raw inflate,
 * payload = original size (4, LE) || raw stream, result = the original bytes; op 'b': one bzip2 stream at block size level (1-9 x 100 KB; streams concatenate).
