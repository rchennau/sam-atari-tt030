...
4. **[`sam-yum-tt`](sam-yum-tt/README.md)** — *alpha*: `yum install` for the TT from a LAN mirror (track `tt030-rpm-pipeline`). The mirror is live and the TT fetches from it with SpareMiNT `wget`. The `sh` + `awk` client is not written yet; its dependency walk is prototyped in `src/closure_probe.awk` (6.1–6.6 s over 415 packages on the 68030). The T425 was measured and not used: hashing at ~1.2× the 68030 does not pay for the link.
