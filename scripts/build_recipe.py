...
also links the package against libt4call.a (t4call.c + t4lock.c + t4adler.c + atwboot.c; header t4call.h on CPPFLAGS), builds
...
def build(name, out_dir, timeout=600, progress=print):
    ...
    try:
        ...
        if t4:
            ...
            for c in ("t4call.c", "t4lock.c", "t4adler.c", "atwboot.c"):
            ...
            run([f"{T}-ar", "rcs", os.path.join(lib, "libt4call.a")] +
                [os.path.join(lib, o) for o in ("t4call.o", "t4lock.o", "t4adler.o", "atwboot.o")], work, env, log, deadline)
